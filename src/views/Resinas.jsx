...
import Card_Resinas from "../components/Card_Resinas";
import arrow_left from "/arrow-left.svg"
import { useNavigate } from "react-router-dom";
function Resinas({data}) {
 ...
 const handleInputChange = (e) => {
    ...
    const matches = data.filter(
      (item) =>
        item.Cod_Interno.toUpperCase().includes(filteredValue) ||
        item.Nombre.toUpperCase().includes(filteredValue)
    );
          ...
          <ul className=" w-[95%] md:max-w-[450px] mx-auto bg-white shadow-md rounded-lg overflow-hidden z-50 divide-y">
            {suggestions.map((item, idx) => (
              <li
                ...
                className="px-4 py-2 cursor-pointer hover:bg-gray-200 text-sm md:text-md"
              >
                <span className="font-bold">{item.Cod_Interno}</span><span className="italic pl-4">{item.Familia}</span> - {item.Nombre}
              </li>
            ))}
          </ul>
        )}
